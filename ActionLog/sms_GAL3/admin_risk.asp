...
'# admin_risk.asp
'#
'# Risk related information; includes visual risk matrix.
'# Not used.
'#
%>
...
<%
'###############################################
'# Retrieve risk information; load information into XML object
'#
%>
<%
...
sql = "select logNumber, division, recid, loginID, createDate, formdataXML from EHD_Data where formname = 'risk' and srtLogNumber = "& sqlnum(log_number) &"  and archived = 'n' and active = 'y' and divisionalLogNumber is null"
...
<%
'###############################################
'# Retrieve log information; load information into XML object
'#
%>
<%
sql = "select * from EHD_Data where formname = 'iSRT_LogInput' and srtLogNumber = "& sqlnum(log_number) &" and archived = 'n' and active = 'y' and divisionalLogNumber is null"
...
<%
'###############################################
'# addTextbox: dynamically add textbox area to pre and post risk sections
'#
%>
...
<form action="admin_submitRiskanalysis.asp" method="post" name="frm" id="frm">
...
<tbody>
<tr><td style="FONT-WEIGHT: bold; FONT-SIZE: 12pt; COLOR: #ffffff; FONT-FAMILY: Arial" align="center" width="100%" background="" bgColor="000080" colSpan="4">Risk Assessment of iSRT Action Log Item</td>
</tr>
...
<td style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial;padding-left:10px;" align="left" width="25%" background="" bgColor="ffffff" >
<%= left(first_name,1) %>&nbsp;<%= last_name %></td>
...
for a = 1 to contributing_factors_cntInt
  dictionaryFields = dictionaryFields &",contributing_factor_"& a
%>
<textarea id="contributing_factor_<%= a %>" name="contributing_factor_<%= a %>" style="width:540px;margin-left:10px;margin-top:5px;" rows="2"><%= rev_xdata2(selectNode(oXML,"contributing_factor_"& a,"")) %></textarea>
<%
next
%>
...
%>
<textarea id="active_error_<%= a %>" name="active_error_<%= a %>" style="width:540px;margin-left:10px;margin-top:5px;" rows="2"><%= rev_xdata2(selectNode(oXML,"active_error_"& a,"")) %></textarea>
<%
next
%>
...
%>
<textarea id="latent_condition_<%= a %>" name="latent_condition_<%= a %>" style="width:540px;margin-left:10px;margin-top:5px;" rows="2"><%= rev_xdata2(selectNode(oXML,"latent_condition_"& a,"")) %></textarea>
<%
next
%>
...
%>
<textarea id="causal_factor_<%= a %>" name="causal_factor_<%= a %>" style="width:540px;margin-left:10px;margin-top:5px;" rows="2"><%= rev_xdata2(selectNode(oXML,"causal_factor_"& a,"")) %></textarea>
<%
next
%>
...
<textarea id="current_measures" name="current_measures" rows="2" required="false" style="width:540px;">
...
<td style="FONT-WEIGHT: normal; FONT-SIZE: 10pt; FONT-FAMILY: Arial;padding-left:10px;" align="left" width="75%" background="" bgColor="ffffff" colspan="3">
<textarea id="corrective_actions" name="corrective_actions" rows="2" required="false" style="width:540px;">
...
<td style="FONT-WEIGHT: normal; FONT-SIZE: 10pt; FONT-FAMILY: Arial;padding-left:10px;" align="left" width="75%" background="" bgColor="ffffff" colspan="3">
<textarea id="unintended_consequences" name="unintended_consequences" rows="2" required="false" style="width:540px;">
...
<tr >
<td align="center">
<input type="submit" value="Submit" style="font-weight:bold;font-size:10px;background-color:white;width:160px;margin-top:5px;margin-left:0px;"><input type="button" value="Return To Log Input" style="font-weight:bold;font-size:10px;background-color:white;width:160px;margin-top:5px;margin-left:0px;" onclick="document.location='admin_LogInput.asp?log_number=<%= log_number %>'"><br><input type="button" value="Return To Action Log" style="font-weight:bold;font-size:10px;background-color:white;width:160px;margin-top:1px;margin-left:0px;" onclick="document.location='admin_LogDisplay.asp'"><br>
<%
dictionaryFields = dictionaryFields &",current_measures,corrective_actions,unintended_consequences"

dim myLink
set myLink = new AspSpellLink
myLink.fields= right(dictionaryFields,len(dictionaryFields)-1) 'Sets the Field id(s) to be spell-checked
