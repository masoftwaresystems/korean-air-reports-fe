...
if(log_number = "") then
  ...
  'sql = "select * from EHD_Data where formname = 'iSRT_LogInput' and srtLogNumber = "& sqlnum(log_number) &" and archived = 'n' order by logNumber desc, recid desc"
  sql = "select * from EHD_Data where formname = 'iSRT_LogInput' and division = "& sqltext2(viewDivision) &" and divisionalLogNumber is null and archived = 'n' and srtLogNumber = "& sqlnum(log_number) &" and active = 'y'"
...
<% if(session("srt_admin") = "y") then %>
  newTD1.innerHTML = "<input type='button' value='delete' style='background-color:white;font-size:8px;margin-right:5px;margin-left:5px;' onclick=\"deleteSafetyAction('"+currCnt+"')\"><span style='FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial;'><%= viewDivision %> <input type='text' name='safety_action_nbr_"+currCnt+"' size='30'  style='width:73px;'  value='<%= log_number %>."+currCnt+"' required='false' readonly /></span>";
<% else %>
  newTD1.innerHTML = "<span style='FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial;'><%= viewDivision %> <input type='text' name='safety_action_nbr_"+currCnt+"' size='30'  style='width:73px;'  value='<%= log_number %>."+currCnt+"' required='false' readonly /></span>";
<% end if %>
...
function calcDueDate(v) {
  ...
  if((opendateVal != "")&&(duedateVal == "")) {
    ...
    dateXML.load("/addDays.asp?date="+opendateVal+"&numdays="+daysToAdd);
...
<td style="FONT-WEIGHT: bold; FONT-SIZE: 8pt;COLOR: #ffffff; FONT-STYLE: normal; FONT-FAMILY: Arial" bgColor="000040" colSpan="1" width="8%" align="center" vAlign="top">Divisional Log Nbr.</td>
...
<%
  srtlinksStr = ""
  sql = "select srtLogNumber from EHD_Data where srtLogNumber = "& sqlnum(log_number) &" and division = "& sqltext2(viewDivision) &" and active = 'y' and divisionalLogNumber is null and formname = 'iSRT_LogInput' order by srtLogNumber asc"
  ...
  if not rs4.eof then
    ...
    sql = "select division, divisionalLogNumber from EHD_Data where srtLogNumber = "& sqlnum(srtLogNumber) &" and active = 'y' and formname = 'iSRT_LogInput' order by division asc, divisionalLogNumber asc"
...
<tr>
<td style="FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial" bgColor="ffffff" colSpan="1" width="8%" align="center"  vAlign="top" rowSpan="1">
<% if(session("srt_admin") = "y") then %><input type="button" value="delete" style="background-color:white;font-size:8px;margin-right:5px;margin-left:5px;" onclick="deleteSafetyAction('<%= a %>')"><% end if %><%= viewDivision %> <input type="text" name="safety_action_nbr_<%= a %>" size="30"  style="width:73px;"  value="<%= log_numberStr %>.<%= a %>" required="false" />
</td>
<td style="FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial" bgColor="ffffff" colSpan="1" width="25%" vAlign="top" rowSpan="1">
...
do while not lrs.eof
  ...
  if(isLocked = "n") then
    clickloc = "admin_LogInput.asp?log_number="& lrs("secondary_log_number") &"&viewDivision="& viewDivision
  else
...
<td align="left"><span style="FONT-WEIGHT: bold; FONT-SIZE: 8pt; FONT-FAMILY: Arial">Existing</span></td>
</tr>
<tr><td >
...
sql = "select divisionalLogNumber, division, recid, loginID, createDate, formdataXML from EHD_Data where formname = 'iSRT_LogInput' and archived = 'n' and active = 'y' and division <> 'iSRT' and divisionalLogNumber is not null and division is not null and division <> '' order by division asc, divisionalLogNumber desc"
set lrs=conn_asap.execute(sql)
do while not lrs.eof
...
%>
  ...
  <td width="100"><input type="checkbox" id="links_<%= lrs("divisionalLogNumber") %>_<%= lrs("division") %>" name="links" value="<%= llog_number %>_<%= lrs("division") %>"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;<%= ldivision %><%= llog_number %></span></td>
  <td width="100%"><span style="FONT-WEIGHT: normal; FONT-SIZE: 10pt; FONT-FAMILY: Arial;border 1px solid black;"><%= litem_description %></span></td></tr>
<%
  end if
  lrs.movenext
loop
...
<tr valign="bottom" height="20">
<td align="left"><span style="FONT-WEIGHT: bold; FONT-SIZE: 8pt; FONT-FAMILY: Arial">New</span></td>
</tr>
<tr><td >
<div style="border:1px solid black;height:100px;overflow:auto;">
<table border="0" cellspacing="5" cellpadding="0" width="200">
<%
sql = "select logNumber from EHD_Data where division = 'ACS' and srtLogNumber = "& sqlnum(log_number) &" and divisionalLogNumber is not null and active = 'y' and archived = 'n' and formName = 'iSRT_LogInput'"
set tmprs=conn_asap.execute(sql)
if tmprs.eof then
%>
  <tr valign="top" style="padding-bottom:10px;">
  <td width="100%"><input type="checkbox" id="links_SRT" name="links" value="ACS"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;ACS</span></td></tr>
<% end if %>
<%
sql = "select logNumber from EHD_Data where division = 'CGO' and srtLogNumber = "& sqlnum(log_number) &" and divisionalLogNumber is not null and active = 'y' and archived = 'n' and formName = 'iSRT_LogInput'"
set tmprs=conn_asap.execute(sql)
if tmprs.eof then
%>
  <tr valign="top" style="padding-bottom:10px;">
  <td width="100%"><input type="checkbox" id="links_SRT" name="links" value="CGO"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;CGO</span></td></tr>
<% end if %>
<%
sql = "select logNumber from EHD_Data where division = 'FOP' and srtLogNumber = "& sqlnum(log_number) &" and divisionalLogNumber is not null and active = 'y' and archived = 'n' and formName = 'iSRT_LogInput'"
set tmprs=conn_asap.execute(sql)
if tmprs.eof then
%>
  <tr valign="top" style="padding-bottom:10px;">
  <td width="100%"><input type="checkbox" id="links_SRT" name="links" value="FOP"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;FOP</span></td></tr>
<% end if %>
<%
sql = "select logNumber from EHD_Data where division = 'IFS' and srtLogNumber = "& sqlnum(log_number) &" and divisionalLogNumber is not null and active = 'y' and archived = 'n' and formName = 'iSRT_LogInput'"
set tmprs=conn_asap.execute(sql)
if tmprs.eof then
%>
  <tr valign="top" style="padding-bottom:10px;">
  <td width="100%"><input type="checkbox" id="links_SRT" name="links" value="IFS"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;IFS</span></td></tr>
<% end if %>
<%
sql = "select logNumber from EHD_Data where division = 'OCC' and srtLogNumber = "& sqlnum(log_number) &" and divisionalLogNumber is not null and active = 'y' and archived = 'n' and formName = 'iSRT_LogInput'"
set tmprs=conn_asap.execute(sql)
if tmprs.eof then
%>
  <tr valign="top" style="padding-bottom:10px;">
  <td width="100%"><input type="checkbox" id="links_SRT" name="links" value="OCC"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;OCC</span></td></tr>
<% end if %>
<%
sql = "select logNumber from EHD_Data where division = 'TOP' and srtLogNumber = "& sqlnum(log_number) &" and divisionalLogNumber is not null and active = 'y' and archived = 'n' and formName = 'iSRT_LogInput'"
set tmprs=conn_asap.execute(sql)
if tmprs.eof then
%>
  <tr valign="top" style="padding-bottom:10px;">
  <td width="100%"><input type="checkbox" id="links_SRT" name="links" value="TOP"><span style="FONT-WEIGHT: bold; FONT-SIZE: 10pt; FONT-FAMILY: Arial">&nbsp;TOP</span></td></tr>
<% end if %>
</table>
</div>
</td>
...
<script>
<%
sql = "select divisionalLogNumber, division from EHD_Data where division is not null and division <> 'iSRT' and srtLogNumber = "& sqlnum(log_number) &" and active = 'y' and archived = 'n' and divisionalLogNumber is not null and formName = 'iSRT_LogInput'"
set lrs = conn_asap.execute(sql)
do while not lrs.eof
%>
document.getElementById("links_<%= lrs("divisionalLogNumber") %>_<%= lrs("division") %>").checked = true;
<%
  lrs.movenext
loop
%>
