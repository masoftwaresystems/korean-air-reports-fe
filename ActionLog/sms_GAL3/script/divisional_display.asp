...
function printPage() {
  ...
  <% else %>
  document.location = "divisional_LogDisplay.asp?overflow=y&searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>&viewDivision=<%= viewDivision %>";
  <% end if %>
}
function printPage2() {
  <% if(overflow = "y") then %>
  ...
  <% else %>
  window.open ("divisional_LogDisplay.asp?overflow=y&searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>&viewDivision<%= viewDivision %>","printwindow");
  <% end if %>
}
<% if(overflow = "y") then %>
printPage2();
...
function switchView() {
  ...
  document.location = "divisional_LogDisplay.asp?searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>&viewDivision=<%= viewDivision %>";
  }
}
...
function doSearch() {
  document.location = "divisional_LogDisplay.asp?searchVal="+ document.getElementById("searchVal").value +"&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>&overflow=<%= overflow %>";
}

function doFilter() {
  document.location = "divisional_LogDisplay.asp?searchVal=<%= searchVal %>&filterSelect="+ document.getElementById("filterSelect").value +"&divisionSelect=<%= divisionSelect %>&overflow=<%= overflow %>";
}

function doDivision() {
  document.location = "divisional_LogDisplay.asp?searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect="+ document.getElementById("divisionSelect").value +"&overflow=<%= overflow %>";
}
