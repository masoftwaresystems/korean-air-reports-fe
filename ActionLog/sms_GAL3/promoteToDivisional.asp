...
if not tmprs.eof then
  ...
  targetXML.selectSingleNode("//resultPage").text = "divisional_LogDisplay.asp"

  sql = "update EHD_Data set lognumber = "& sqlnum(maxlog_number) &", divisionalLogNumber = "& sqlnum(maxlog_number) &" where srtstagenumber = "& sqlnum(log_number) &" and division = "& sqltext2(viewDivision) &" and archived = 'n' and active = 'y' and formname = 'iSRT_LogInput'"
  conn_asap.execute(sql)

  createFormDataXML session("loginID"), request("formname"), targetXML.xml, maxlog_number, viewDivision, ""

end if


response.redirect "divisional_LogDisplay.asp?viewdivision="& viewDivision
response.end
%>