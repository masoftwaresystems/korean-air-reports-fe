...
if(searchVal <> "") then
    sql = "select divisionalLogNumber, srtlognumber, logNumber, division, recid, loginID, createDate, formdataXML from EHD_Data where formname = 'iSRT_LogInput' and active = 'y' and divisionalLogNumber is not null order by logNumber desc, recid desc"
else
...
' }}} %>
        ...
        <form name="frm" id="frm" action="divisional_LogDisplay.asp" method="post">
          <table cellSpacing="0" cellPadding="1" width="100%" border="1">
            <tr class="LogHead">
              <td width="5%">Hazard ID</td>
              <td width="24%">Title/Description</td>
              <td width="5%">Divisional</td>
...
<% if(isLocked <> "n") then %>
                ...
                <span onclick="<%= clickloc %>"><%= hazard_id %>&nbsp;</span>
...
<% end if %>
              ...
              <td onclick="<%= clickloc %>" style="text-align:left;padding-right:10pt;padding-bottom:4px;">
                <span style="color:<%= lateColor %>;"><u><%= item_title %></u></span><% if(item_description <> "") then %>&nbsp;&nbsp;<%= item_description %><% end if %>&nbsp;
              </td>
              <td onclick="<%= clickloc %>">
                <%= srtlinksStr %>&nbsp;
              </td>
...
' }}} %>
            ...
            <tr class="ItemHead" onclick="<%= clickloc %>"  onmouseover="setRowColor2(this, '#f0f0f0');this.style.cursor='hand';" onmouseout="setRowColor2(this, 'white')" style="font-weight:normal;text-align:center;background-color: #ffffff">
              ...
              <td>
                <%= safety_action_openStr %>&nbsp;
              </td>
              <td colspan="2">
                <%= safety_action_dueStr %>&nbsp;
              </td>
              <td>
                <%= safety_action_completedStr %>&nbsp;
              </td>
              <td>
<% ' {{{ Check if completed
...
' }}} %>
            ...
            <tr><td colspan="12" style="height:1px;background-color:silver;"></td></tr>
            <!--
            <tr>
              <td bgcolor="#ffffff" colspan="12" >
                <div style="height:1px;font-weight:normal;border-top:1px solid black;font-size:0px;background-color:#000040;">
                  &nbsp;
                </div>
              </td>
            </tr>
...
%>
<span style="width:20px;text-align:center;"><a href="divisional_LogDisplay.asp?position=<%= tmppos %>&filterSelect=<%= filterSelect %>&searchVal=<%= searchVal %>&divisionSelect=<%= divisionSelect %>&viewDivision=<%= viewDivision %>"><%= pagenum %></a></span>
<%
      end if
      tmpcnt = tmpcnt - 10
    loop
