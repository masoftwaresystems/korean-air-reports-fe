...
<table border="0" cellspacing="0" cellpadding="0" width="550" style="padding-top:20px;margin-left:10px;">
  <tr><td align="center"><span class="fonts4" style="font-size:14px;color:#737579;">Hazards </span></td></tr>
</table>
...
%>
		<tr valign="bottom" height="20">
		  <td align="left" style="padding-left:40px;"><a href="divisional_Base.asp?viewdivision=<%= viewdivision %>&viewtype=<%= viewtype %>&base=GEN"><img src="http://www.bristowsafety.com/sms_images/collapse.gif" border="0"><span style="font-weight: bold; font-size: 10pt;"><%= viewDivision %></span></a></td>
		</tr>
<%
  end if
%>
		...
		<tr>
		  <td align="left"><span style="padding-left:80px;font-weight: bold; font-size: 8pt;"><% if(session("sms_admin") = "y") then %><input type="button" value="delete" style="font-size:10px;font-weight:normal;" onclick="showConfirm('<%= hazard_id %>',this)" id="BTN_<%= hazard_id %>"> <% end if %><a href="getSummary.asp?log_number=<%= log_number%>&viewdivision=<%= viewdivision %>&framed=y" target="detailFrame"><%= hazard_id %> : <%= item_title %></a>
