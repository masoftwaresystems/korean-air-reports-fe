...
var s_account="sfvisitorslive"
var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="sfvisitors"
s.trackingServer="c.onlyinsanfrancisco.com"
s.trackingServerSecure="b.onlyinsanfrancisco.com"
s.dc="122"
...
s.linkInternalFilters="javascript:,onlyinsanfrancisco.com,sfcvb.org,welcometosf.com,localhost,sanfrancisco.travel,dineabouttown.com"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="prop5,prop6,prop7,prop8"
...
s._channelPattern="Paid Search|knc-,ps->Banner Ad|bac-,ba-,eMail|em-,emc->Vanity Redirect|va->Paid Social Media|sm->Affiliates|affil->Tagged Social Media|tsm-"
/* 
Marketing Channel		Begin campaign value with:		example
 
Paid Search				knc- or ps-						knc-1101-google_abc
Banner Ad				bac- or ba-						ba-1105-thompsonrouter
Email					emc- or em-						emc-110115-legalnews
Vanity Redirect			va-								va-1101-redirect
Paid Social Media		sm-								sm-1101-facebook_abc
Affiliates				affil-							affil-aaaLawFirm
...
function s_doPlugins(s) {
  ...
  s.channel = pageNameArray[0];
  s.prop1 = s.channel + ": " + pageNameArray[1];
  s.hier2='D=pageName';
  s.eVar7='D=pageName';

  if (document.URL.indexOf('onlyinsanfrancisco.com') > -1) 
  {
    s.server="onlyinsanfrancisco.com";
  } 
  else if (document.URL.indexOf('sfcvb.org') > -1) 
  {
    s.server="sfcvb.org";
  }
  else if (document.URL.indexOf('welcometosf.com') > -1) 
  {
    s.server="welcometosf.com";
  }
  if(s.server)
  {
	  s.eVar6 = s.server
  ...
  //Collect Browse Category from the URL QUERY PARAMETER Category_Code	
  if (!s.eVar42)
  {
	  s.eVar42 = s.getQueryParam('Category_Code');//Set campaign here if not set in page already.
	  s.eVar42=s.getValOnce(s.eVar42,'s_eVar42',0);
  }
  else 
  {
	  s.eVar42=s.getValOnce(s.eVar42,'s_eVar42',0); 
  ...
  s.prop21=s.eVar21=s.getNewRepeat();
  if(s.pageName && s.prop21 == 'New') s.prop22 = s.pageName;
  if(s.pageName && s.prop21 == 'Repeat') s.prop23 = s.pageName;
		
	  /* Set Time Parting Variables */
	  var currentDate = new Date()
	  var year = currentDate.getFullYear()
	  s.prop17 = s.getTimeParting('h','-8',year).toLowerCase(); // Set hour, set to pacific time zone
	  s.eVar17 = 'D=c17';
	  s.prop18 = s.getTimeParting('d','-8',year).toLowerCase(); // Set day, set to pacific time zone
	  s.eVar18 = 'D=c18';
	...
	/* Get Days Since Last Visit */
		s.prop20=s.getDaysSinceLastVisit('s_dayslastvisit');
		s.eVar20="D=c20";
  /* External Campaign Tracking */
	  if(!s.campaign)
		  {
			  s.campaign=s.getQueryParam('cmpid,cmp,rss,sharecode,src,affil');//Set campaign here if not set in page already.
	  ...
	  if(!s.eVar8)
		  {
			  s.eVar8=s.getQueryParam('intcmp');//Set internal campaign here if not set in page already.
	  ...
	  /* Site Search */
		if(s.prop10)
		{
			s.prop10 = s.getValOnce(s.prop10,'search_prop10',0);
			if(s.prop10)
			{
				s.events=s.apl(s.events,"event5",",",2);
				s.eVar10='D=c10';
				s.prop45 = s.getPreviousValue(s.pageName,'qpv_pagename','');
				s.eVar45 = "D=c45";
			}
		}
	 
	  /*********************
	   *prop31 is Featured Content
	   *eVar31 is Featured Content
	   *event14 is Featured Content Clicks
	  *********************/
	  if(s.prop31){
		s.eVar31=s.prop31=s.prop31.toLowerCase();
		var t_search=s.getValOnce(s.eVar31,'ev14',0);
		if(t_search) {
		  if(s.events)
	...
	///* Automatically Capture Exit Links */
		...
		if(s.downloadURL)
		{
			s.linkTrackVars = "prop5,prop6,prop7,prop8";
			s.setupLinkTrack("prop5,prop6,prop7,prop8","SC_LINKS");
			s.linkTrackVars = s.apl(s.linkTrackVars,"prop16,eVar16,events",",",2);
		...
		if(s.prop43)
		{
			s.prop43 = s.getValOnce(s.prop43,'search_prop43',0);
			if(s.prop43)
			{
	...
	/* Internal Search Browse */
		if(s.prop44)
		{
			s.prop44 = s.getValOnce(s.prop44,'search_prop44',0);
			if(s.prop44)
			{
	...
	/* Set Channel Variables */
		s.channelManager('cmp,rss,sharecode,src,affil');
		if(s._channel)
		{
			s.prop47 = s._channel;
			s.eVar47 = "D=c47";
			...
			{
				s.prop48 = s.eVar48 = 'na';
			}			
			if(s._keywords)
			{
				s.prop49 = s._keywords;
				s.eVar49 = "D=c49";
			}
			else
			{
				s.prop49 = s.eVar49 = 'na';
			}
			
			if(s._referringDomain)
			{
				s.prop50 = s._referringDomain;
				s.eVar50 = "D=c50";
			}
			else
			{
				s.prop50 = s.eVar50 = 'na';
			}
// Lines commented as a request from David Humphrey on March 11, 2011
//			s.hier3=s.crossVisitParticipation(s.campaign,'s_campaign_channel','30','15','>','',1);
//			s.hier4=s.crossVisitParticipation(s.eVar47,'s_ev47_channel','30','15','>','',1);
		  ...
		  if(s.prop26&&!s.eVar26) s.eVar26='D=c26';
		  if(s.prop27&&!s.eVar27) s.eVar27='D=c27';
		  if(s.prop28&&!s.eVar28) s.eVar28='D=c28';
		  if(s.prop29&&!s.eVar29) s.eVar29='D=c29';
