...
%>
<!--
session("loginID"):<%= session("loginID") %>
Request.Cookies("loginID"):<%= Request.Cookies("loginID") %>
-->
...
<table border="0" cellspacing="0" cellpadding="0" width="98%" style="padding-top:0px;margin-left:0px;">
  <tr><td align="center"><span class="fonts4" style="font-size:14px;color:#737579;"><span style="font-weight:bold;color:#116c94;">Hazard Dashboard</span></span><div id="PINWHEEL" style="display:none;"><img src="/miat/images/pinwheel.gif">
</div></td></tr>
</table>
-->
<center>
...
<table border="0" cellspacing="1" cellpadding="0" width="800" bgcolor="silver">
	<tr>
		...
		<td bgcolor="white" width="50" align="left"><img src="/miat/images/1pyellow.png" height="10" width="20" style="border:1px solid black;"></td>
		<td bgcolor="white" width="150" align="right">Overdue&nbsp;</td>
		<td bgcolor="white" width="50" align="left"><img src="/miat/images/1pred.png" height="10" width="20" style="border:1px solid black;"></td>
		<td bgcolor="white" width="150" align="right">Closed&nbsp;</td>
		<td bgcolor="white" width="150" align="left"><img src="/miat/images/1pgreen.png" height="10" width="20" style="border:1px solid black;"></td>
	</tr>
</table>
...
if((next_review_date <> "") and (not isnull(next_review_date)) and (isdate(next_review_date))) then
  ...
  if((date_completed <> "") and (not isnull(date_completed)) and (isdate(date_completed)) and (isdate(date_opened))) then
...
<table border="0" cellspacing="1" cellpadding="0" width="500" bgcolor="silver" style="margin-top:30px;">
	...
	<tr>
		<td bgcolor="white" width2="250" align="center"><% if(greenwidth > 0) then %><img src="/miat/images/1pgreen.png" height="10" width="<%= greenwidth %>" style="border:1px solid black;"><% end if %><% if(yellowwidth > 0) then %><img src="/miat/images/1pyellow.png" height="10" width="<%= yellowwidth %>" style="border:1px solid black;"><% end if %><% if(redwidth > 0) then %><img src="/miat/images/1pred.png" height="10" width="<%= redwidth %>" style="border:1px solid black;"><% end if %><!-- <br><%= total_hazards %>|<%= endorsed_hazards %>|<%= loaded_hazards %>|<%= late_hazards %> --></td>
	</tr>
</table>
<%
  ors.movenext
...
<%
sql = "select * from Bullet_info_t where REPORTER = 'all'"
set rs = conn_asap.execute(sql)
if not rs.eof then
%>
<center>
...
<br><br>
<%
sql = "select * from Bullet_info_t where REPORTER = "& sqltext2(viewdivision)
set rs = conn_asap.execute(sql)
if not rs.eof then
%>
<center>
