...
<% if(request("cookie_sms_admin") = "y") then %>
...
function goToRisk() {
  //frm.action = "divisional_risk2.aspx";
...
<table border="0" cellspacing="0" cellpadding="0" width="98%" style="padding-top:0px;margin-left:0px;">
  <tr><td align="center"><span class="fonts4" style="font-size:14px;color:#737579;"><span style="font-weight:bold;color:#116c94;">Hazard ID <%= hazard_id %></span></span><div id="PINWHEEL" style="display:none;"><img src="http://www.bristowsafety.com/images/pinwheel.gif">
</div></td></tr>
</table>
...
<% end if %>
<input name="item_title" style="width:550px;" <% if((request("cookie_sms_admin") <> "y") and (request("log_number") <> "")) then %>readOnly<% end if %> value="<%= item_title %>"></td>
</tr>
</table>
              </td>
<%
  ...
  if((is_iSRT = "y") and (request("cookie_srt_admin") <> "y")) then
...
<textarea name="item_description" style="width:600px;" rows="8" <% if((request("cookie_sms_admin") <> "y") and (request("log_number") <> "")) then %>readOnly<% end if %> <%= srtReadonly %> ><%= item_description %></textarea>
</td>
...
<%= baseStr %>
<% else %>
<input type="text" name="baseStr" id="baseStr" style="width:230px;" <% if((request("cookie_sms_admin") <> "y") and (request("log_number") <> "")) then %>readOnly<% end if %> value = "<%= baseStr %>">
<% end if %>
</td>
...
<%= equipment %>
<% else %>
<input type="text" name="equipment" id="equipment" style="width:130px;" <% if((request("cookie_sms_admin") <> "y") and (request("log_number") <> "")) then %>readOnly<% end if %> value = "<%= equipment %>">
<% end if %>
</td>
...
<%= mission %>
<% else %>
<input type="text" name="mission" id="mission" style="width:130px;" <% if((request("cookie_sms_admin") <> "y") and (request("log_number") <> "")) then %>readOnly<% end if %> value = "<%= mission %>">
<% end if %>
</td>
...
<input type="text" id="date_due" name="date_due" size="30"  style="width:73px;"  value="<%= date_due %>"   <%= srtReadonly %> required2 onblur="validateDate(this)" <% if(request("cookie_sms_admin") <> "y") then %>readOnly<% end if %>  title="<%= date_dueStr %>" />
</td><td align="left" width="50%"><% if(request("cookie_sms_admin") = "y") then %><a onclick="javascript:return isCalendarLoaded()" href="javascript:openCalendar('calendar2', getCurrentDate(), getNextYear(), 'frm', 'date_due')"><img id="calendar2" src="http://www.atlanta.net/global_images/appts.png" border="0"><% end if %></a>
</td></tr>
</table>
</td>
...
<td style="FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial" bgColor="ffffff"  colSpan="1" width="8%" align="center" vAlign="top" rowSpan="1">
<input type="text" id="next_review_date" name="next_review_date" size="30"  style="width:73px;"  value="<%= next_review_date %>"  onblur2="if(validateDate(this)){calcDueDate('');}"  <%= srtReadonly %> required2 title="<%= next_review_dateStr %>" <% if(request("cookie_sms_admin") <> "y") then %>readOnly<% end if %> />
</td>
<td style="FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial" bgColor="ffffff"  colSpan="1" width="8%" align="center" vAlign="top" rowSpan="1">
</td>
<td style="FONT-WEIGHT: normal; FONT-SIZE: 8pt;  FONT-STYLE: normal; FONT-FAMILY: Arial" bgColor="ffffff"  colSpan="2" width="8%" align="center" vAlign="top" rowSpan="1">
...
<% if(request("cookie_sms_admin") = "y") then %><input type="button" value="delete" style="font-size:8px;margin-right:5px;margin-left:5px;" onclick="deleteSafetyAction('<%= a %>')"> <% end if %><%= viewDivision %><input type="text" id="safety_action_nbr_<%= a %>" name="safety_action_nbr_<%= a %>" size="15"  style="width:60px;"  value="<%= log_numberStr %>.<%= a %>"  /><br/>
...
do while not lrs.eof
  ...
  if(isLocked = "n") then
    clickloc = "divisional_LogInput2.aspx?log_number="& lrs("secondary_log_number") &"&viewDivision="& viewDivision
  else
...
<% if((request("cookie_srt_admin") = "l") or ((request("cookie_srt_admin") = "p"))) then %>
<% if(is_iSRT = "y") then %>
...
<% end if %>
<% end if %>
<% if(request("cookie_srt_admin") = "y") then %>
<% if(is_iSRT = "y") then %>
...
function sendPromotionRequest() {
  var alerttext = "Request Sent\nCopy of request sent from <%= request("cookie_email_address") %>";
  var currNode;
  isrtXML.async = false;
  isrtXML.resolveExternals = false;
  isrtXML.load("/requestPromotionToSRT.asp?logNumber=<%= log_numberStr %>&viewDivision=<%= viewDivision %>&requestor=<%= request("cookie_employee_number") %>");
  alert(alerttext);
...
function sendDemotionRequest() {
  ...
  isrtXML.async = false;
  isrtXML.resolveExternals = false;
  isrtXML.load("/requestDemotionFromSRT.asp?logNumber=<%= log_numberStr %>&viewDivision=<%= viewDivision %>&requestor=<%= request("cookie_employee_number") %>");
  alert(alerttext);
  document.location='divisional_LogDisplay.asp?viewdivision=<%= viewdivision %>';
}
