...
sql = "select NVL(max(logNumber),0) maxlogNumber from EHD_Data where division = "& sqltext2(divisionStr)
...
if(obj_exists(oXML.selectSingleNode("//accountable_leader"))) then
  ...
  sql = "insert into EHD_Data (recid, loginID, createDate, division, formName, formDataXML, logNumber, archived, modifyDate, modifyUser, lockedrecord, lockedDate, lockedUser, active, srtLogNumber, divisionalLogNumber, srtStageNumber, base, equipment, generic, hazard_base, hazard_type, hazard_number) values ("& maxid &","& sqlnum(rs("loginID")) &",current_date,"& sqltext2(trim(divisionStr)) &","& sqltext2(rs("formName")) &","& sqltext2(formDataXML) &","& sqlnum(log_number) &","& sqltext2(rs("archived")) &",current_date,"& sqltext2(rs("modifyUser")) &","& sqltext2(rs("lockedrecord")) &","& sqltext2(rs("lockedDate")) &","& sqltext2(rs("lockedUser")) &","& sqltext2(rs("active")) &","& sqlnum(rs("srtLogNumber")) &","& sqlnum(log_number) &","& sqlnum(rs("srtStageNumber")) &","& sqltext2(trim(divisionStr)) &","& sqltext2(tmpEquipment) &",'y',"& sqltext2(baseCode) &","& sqltext2(trim(divisionStr)) &","& sqlnum(generic_log_number) &")"
  ...
  maxid = cint(maxid) +1
    ...
    sql = "insert into EHD_Data (recid, loginID, createDate, division, formName, formDataXML, logNumber, archived, modifyDate, modifyUser, lockedrecord, lockedDate, lockedUser, active, srtLogNumber, divisionalLogNumber, srtStageNumber, base, equipment, generic, hazard_base, hazard_type, hazard_number) values ("& maxid &","& sqlnum(tmprs("loginID")) &",current_date,"& sqltext2(trim(divisionStr)) &","& sqltext2(tmprs("formName")) &","& sqltext2(formDataXML) &","& sqlnum(log_number) &","& sqltext2(tmprs("archived")) &",current_date,"& sqltext2(tmprs("modifyUser")) &","& sqltext2(tmprs("locked")) &","& sqltext2(tmprs("lockedDate")) &","& sqltext2(tmprs("lockedUser")) &","& sqltext2(tmprs("active")) &","& sqlnum(tmprs("srtLogNumber")) &","& sqlnum(log_number) &","& sqlnum(tmprs("srtStageNumber")) &","& sqltext2(trim(divisionStr)) &","& sqltext2(tmpEquipment) &",'y',"& sqltext2(baseCode) &","& sqltext2(trim(divisionStr)) &","& sqlnum(generic_log_number) &")"
    response.write("sql:"& sql &"<br>")
    conn_asap.execute(sql)
