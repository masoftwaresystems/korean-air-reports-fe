...
if(request("searchStr") <> "") then
  if(request("cookie_seeDivision") = "y") then
    sql = "select divisionalLogNumber, srtlognumber, logNumber, division, recid, loginID, createDate, formdataXML from EHD_Data where formname = 'iSRT_LogInput' and active = 'y' and divisionalLogNumber is not null and  upper(formdataXML) like  upper('%"& request("searchStr") &"%') and division = "& sqltext2(request("cookie_division")) &" order by logNumber desc, recid desc"
  else
    sql = "select divisionalLogNumber, srtlognumber, logNumber, division, recid, loginID, createDate, formdataXML from EHD_Data where formname = 'iSRT_LogInput' and active = 'y' and divisionalLogNumber is not null and  upper(formdataXML) like  upper('%"& request("searchStr") &"%') order by logNumber desc, recid desc"
  end if
...
if(request("searchStr") <> "") then
  if(request("cookie_seeDivision") = "y") then
    sql = "select count(*) cnt from EHD_Data where formname = 'iSRT_LogInput' and active = 'y' and divisionalLogNumber is not null and  upper(formdataXML) like  upper('%"& request("searchStr") &"%') and division = "& sqltext2(request("cookie_division")) &" order by logNumber desc, recid desc"
  else
    sql = "select count(*) cnt from EHD_Data where formname = 'iSRT_LogInput' and active = 'y' and divisionalLogNumber is not null and  upper(formdataXML) like  upper('%"& request("searchStr") &"%') order by logNumber desc, recid desc"
  end if

else
    sql = "select count(*) cnt from EHD_Data where formname = 'iSRT_LogInput' and active = 'y' and division = "& sqltext2(viewDivision) &" and divisionalLogNumber is not null order by logNumber desc, recid desc"
end if
