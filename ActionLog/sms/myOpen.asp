...
sql = "select main.formDataXML formDataXML, main.logNumber, main.divisionalLogNumber divisionalLogNumber, main.division division, risk.formDataXML formDataXML2 from EHD_Data main left outer join EHD_Data risk on main.divisionalLogNumber = risk.divisionalLogNumber and main.division = risk.division  where main.division = "& sqltext2(request("cookie_division")) &" and main.formName = 'iSRT_LogInput' and main.active = 'y' and risk.active = 'y' and risk.formName = 'risk' order by main.divisionalLogNumber desc, main.recid desc"
set rs = conn_asap.execute(sql)
do while not rs.eof
