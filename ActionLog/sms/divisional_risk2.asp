...
if(base = "GLOBAL") then
  ...
  sql = "select distinct loginID, username, first_name, last_name from Tbl_Logins where ((division = "& sqltext2(viewdivision) &") or (sms_admin = 'y')) and last_name <> 'Aaron' order by last_name asc"
end if
set tmprs=conn_asap.execute(sql)
...
acXML.async					= false
sql = "select * from EHD_Comments, Tbl_Logins where EHD_Comments.division = "& sqltext2(viewdivision) &" and log_number = "& sqlnum(log_number) &" and comment_type = 'ActionItem_"& a &"' and EHD_Comments.loginID = Tbl_Logins.loginID order by recid desc"
set tmprs = conn_asap.execute(sql)
do while not tmprs.eof
...
%>
<p><span decode><%= tmpdateStr %>&nbsp;<%= tmprs("first_name") %>&nbsp;<%= tmprs("last_name") %><br></span><span decode><%= selectNode(acXML,"comment","") %></span></p>
<%
  end if
  tmprs.movenext
loop
...
cXML.async					= false
sql = "select * from EHD_Comments, Tbl_Logins where EHD_Comments.division = "& sqltext2(viewdivision) &" and log_number = "& sqlnum(log_number) &" and comment_type = 'risk' and EHD_Comments.loginID = Tbl_Logins.loginID order by recid desc"
set tmprs = conn_asap.execute(sql)
do while not tmprs.eof
  if(len(tmprs("item_commentXML")) > 0) then
...
function addNew(t,r) {
  ...
  $('#safety_action_open_'+currCnt).datepicker();
  $('#safety_action_due_'+currCnt).datepicker();
  $('#safety_action_completed_'+currCnt).datepicker();
