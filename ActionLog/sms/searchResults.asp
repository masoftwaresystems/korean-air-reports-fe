...
<script type="text/javascript">
	...
	<% else %>
	document.location = "searchResults.asp?overflow=y&searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>";
	<% end if %>
    }
    function printPage2() {
	<% if(overflow = "y") then %>
	...
	<% else %>
	window.open ("searchResults.asp?overflow=y&searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>","printwindow");
	<% end if %>
    }
    <% if(overflow = "y") then %>
    printPage2();
	...
	if(overflow != "y") {
	    ...
	    document.location = "searchResults.asp?searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>";
	}
    }
</script>
<style>
...
function doSearch() {
  document.location = "searchResults.asp?searchVal="+ document.getElementById("searchVal").value +"&filterSelect=<%= filterSelect %>&divisionSelect=<%= divisionSelect %>&overflow=<%= overflow %>";
}

function doFilter() {
  document.location = "searchResults.asp?searchVal=<%= searchVal %>&filterSelect="+ document.getElementById("filterSelect").value +"&divisionSelect=<%= divisionSelect %>&overflow=<%= overflow %>";
}

function doDivision() {
  document.location = "searchResults.asp?searchVal=<%= searchVal %>&filterSelect=<%= filterSelect %>&divisionSelect="+ document.getElementById("divisionSelect").value +"&overflow=<%= overflow %>";
}
...
sql = "select logNumber, division, recid, loginID, createDate, formdataXML from EHD_Data where formname = 'iSRT_LogInput' and archived = 'n' and active = 'y' order by logNumber desc, recid desc"
set rs = conn_asap.execute(sql)

%>

<form name="frm" id="frm" action="searchResults.asp" method="post">
...
%>
<% if(firstOne <> "y") then %>
<tr><td bgcolor="#ffffff" colspan="11" ><div style="height:0px;border:1px solid black;font-size:1px;background-color:#000040;">&nbsp;</div></td></tr>
<% else %>
<% firstOne = "n" %>
<% end if %>
<%
...
<tr height="0">
<td align="center">
<span style="width:150px;font-weight:bold;"><% if(startpos > 0) then %><a href="searchResults.asp?position=<%= prevpos %>&filterSelect=<%= filterSelect %>&searchVal=<%= searchVal %>&divisionSelect=<%= divisionSelect %>">&lt;&lt;prev</a><% else %>&lt;&lt;prev<% end if %></span>
<span style="width:150px;font-weight:bold;"><% if(recsshown = 11) then %><a href="searchResults.asp?position=<%= currpos %>&filterSelect=<%= filterSelect %>&searchVal=<%= searchVal %>&divisionSelect=<%= divisionSelect %>">next&gt;&gt;</a><% else %>next&gt;&gt;<% end if %></span>
</td>
</tr>
